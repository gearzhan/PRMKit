...
// Level 1管理员权限（全权限）
export const requireLevel1Admin = authorizeRoles('DIRECTOR', 'ASSOCIATE', 'OFFICE_ADMIN');

// Level 2经理权限（时间表权限）- 包括Level 1管理员
export const requireLevel2Manager = authorizeRoles('DIRECTOR', 'ASSOCIATE', 'OFFICE_ADMIN', 'PROJECT_MANAGER');

// Level 3员工权限（时间表权限）
export const requireLevel3Worker = authorizeRoles('JUNIOR_ARCHITECT', 'ARCHITECT');

// 时间表访问权限（Level 2和Level 3）
export const requireTimesheetAccess = authorizeRoles(
  'PROJECT_MANAGER', 'JUNIOR_ARCHITECT', 'ARCHITECT'
);
